...
function Map() {
    ...
    return (
      <>
        ...
        <ul>
          {itemsColor.map((data, index)=><li key={index}  data-atr={data}>
            {data}
          </li>)} 
        </ul>
        <h1>Only Red Color</h1>
        <ul>
          {itemsColor.map((data, index)=><li key={index}> 
            {itemsColor[0]}
          </li>)} 
        </ul>
        <h1>Filter Only Red Color</h1>
        <ul>
          {itemsColor.filter((color) => color === "Red").map((color, index) => (
            <li key={index} data-atr={color}>{color}</li>
          ))} 
 
        </ul>

      <ul className="bio-block">
        {itemsObj.map((data, index)=><li key={index}>
          <span data-id={data.id}><b>ID:</b> {data.id}</span><br />
          <span data-id={data.Name}><b>Name:</b> {data.Name}</span><br />
          <span data-id={data.Surname}><b>Surname:</b> {data.Surname}</span><br />
          <span data-id={data.Age}><b>Age:</b> {data.Age}</span><br />
        </li>)}
      </ul>
      </>
    );
      ...
      <div>
        {filteredData.map((data) => (
          <div key={data.id} className="productmain">            
            <div><label>Product Name : </label>{data.name}</div>
            <div><label>Product Category :</label>{data.category}</div>
            <div><label>Product Price :</label>{data.price}</div>
          </div>
        ))}
      </div>
</div>
